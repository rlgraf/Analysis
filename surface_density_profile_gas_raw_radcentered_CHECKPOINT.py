#!/usr/bin/env python3
#SBATCH --job-name=surface_density_profile_gas_raw_radcentered_CHECKPOINT
...
#SBATCH --output=surface_density_profile_gas_raw_radcentered_CHECKPOINT_%j.txt
...
def surf_dens_log_frac(x1,x2,x3,x4,r,part,particle_thresh = 4):
    ...
    if len(part['gas']['mass'][index2]) < particle_thresh:
           return(np.nan)
           
    surf_dens_lookback = np.sum(part['gas']['mass'][index2])/(np.pi*(x2**2 - x1**2))
...
### m12i
    ...
    ut_io.file_hdf5('/home/rlgraf/Final_Figures/surface_density_profile_gas_raw_radcentered_CHECKPOINT', surf_dens_ratio_gas_all_galaxies)
