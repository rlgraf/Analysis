#!/usr/bin/env python3
#SBATCH --job-name=ninety_mass_radius_analysis_percentile_form_CHECKPOINT
...
#SBATCH --output=ninety_mass_radius_analysis_percentile_form_CHECKPOINT_%j.txt
...
def half_mass_radius_form(x1,x2,x3,x4,a1,a2,r_form_cylindrical,r_form_spherical,age,part):
    ...
    if len(r_form_cylindrical[:,0][index3]) == 0:
        ...
        mass_cut = ut.math.percentile_weighted(r_form_cylindrical[:,0][index3], 90, weights = part['star']['mass'][index3])
        return(mass_cut)
    ...
    for q, s in enumerate(sim):
        ...
        for j, (r_spherical, r_form_spherical,r_cylindrical, r_form_cylindrical) in enumerate(zip(r_array_spherical,r_form_array_spherical,r_array_cylindrical,r_form_array_cylindrical)):
            ...
            for a in np.arange(0,14):
                half_mass_radius_at_age.append(half_mass_radius_form(-3,3,0,30,a,14,r_form_cylindrical,r_form_spherical,age,part))
            half_mass_radius_galaxy.append(half_mass_radius_at_age)
    half_mass_radius_galaxy = np.array(half_mass_radius_galaxy)
    ut_io.file_hdf5('/home/rlgraf/Final_Figures/90_mass_radius_form_CHECKPOINT', half_mass_radius_galaxy)
