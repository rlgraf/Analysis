#!/usr/bin/env python3
#SBATCH --job-name=half_mass_radius_analysis_percentile_form
...
#SBATCH --output=half_mass_radius_analysis_percentile_form_%j.txt
...
def half_mass_radius_form(x1,x2,x3,x4,a1,a2,r,r_form,age,part):
    ...
    mass_cut = ut.math.percentile_weighted(r_form[:,0][index3], 95, weights = part['star']['mass'][index3])
    ...
    for q, s in enumerate(sim):
        ...
        for j, (r, r_form) in enumerate(zip(r_array,r_form_array)):
            ...
            for a in np.arange(0,14):
                half_mass_radius_at_age.append(half_mass_radius_form(-3,3,0,20,a,a+0.1,r,r_form,age,part))
            half_mass_radius_galaxy.append(half_mass_radius_at_age)
    half_mass_radius_galaxy = np.array([half_mass_radius_galaxy])
    ut_io.file_hdf5('/home/rlgraf/Final_Figures/95_mass_radius_form_spherical_young_v2', half_mass_radius_galaxy)
