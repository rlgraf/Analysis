#!/usr/bin/env python3
#SBATCH --job-name=radial_analysis_r_0_5
...
#SBATCH --output=radial_analysis_r_0_5_%j.txt
...
def radial_analysis_z_0():
        ...
        for r, r_form in zip(r_array,r_form_array):
            ...
            for a in np.arange(0,14):
                ...
                for i in np.arange(0,5,0.5):
                    x.append(Fe_H_agedependent(i,i+0.5,-3,3,0,5,-3,3,a,a+1,r,r_form,age,part))
                Fe_H_rad.append(x)
                l = np.arange(0,5,0.5)
                x = np.array(x)
    ...
    ut_io.file_hdf5('/home/rlgraf/Final_Figures/RAD_profile_z_0_r_0_5', Fe_H_rad_total)
    ut_io.file_hdf5('/home/rlgraf/Final_Figures/RAD_slope_z_0_r_0_5', slope_total)
...
def radial_analysis_form():
    ...
    for s in sim:
        ...
        for r, r_form in zip(r_array,r_form_array):    
            ...
            for a_f in np.arange(0,14):
                x_f = []
                for i_f in np.arange(0,5,0.5):
                    x_f.append(Fe_H_agedependent_form(i_f,i_f+0.5,-3,3,0,5,-3,3,a_f,a_f+1,r_form,r,age,part))
                Fe_H_rad_form.append(x_f)
                l_f = np.arange(0,5,0.5)
                x_f = np.array(x_f)
    ...
    ut_io.file_hdf5('/home/rlgraf/Final_Figures/RAD_profile_form_r_0_5', Fe_H_rad_form_total)
    ut_io.file_hdf5('/home/rlgraf/Final_Figures/RAD_slope_form_r_0_5', slope_form_total)
