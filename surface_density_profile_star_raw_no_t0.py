#!/usr/bin/env python3
#SBATCH --job-name=surface_density_profile_star_raw_no_t0
...
#SBATCH --output=surface_density_profile_star_raw_no_t0_%j.txt
...
def surf_dens_log_frac(x1,x2,x3,x4,x5,x6,x7,x8,r,r_z0,part,part_z0,particle_thresh = 4):
    ...
    surf_dens_z0 = np.sum(part_z0['star']['mass'][index2])/(np.pi*(x2**2 - x1**2))
    
    index3 = ut.array.get_indices(r[:,0], [x5,x6])
    index4 = ut.array.get_indices(abs(r[:,2]), [x7,x8], prior_indices = index3)
    surf_dens_lookback = np.sum(part['star']['mass'][index4])/(np.pi*(x6**2 - x5**2))
...
### m12i
    ...
    part_snapshots = np.array([0.20076481, 0.47764710, 0.90881461, 1.73913038])
    for red in part_snapshots:
        part_z0 = gizmo.io.Read.read_snapshots(['star'], 'redshift', 0, simulation_directory, properties = ['mass', 'position'], assign_hosts_rotation=True)
        part = gizmo.io.Read.read_snapshots(['star'], 'redshift', red, simulation_directory, properties = ['mass', 'position'], assign_hosts_rotation=True)
        
        r_z0 = part_z0['star'].prop('host.distance.principal.cylindrical')
    ...
    for red in part_snapshots:
        ...
        r_z0 = part_z0['star'].prop('host1.distance.principal.cylindrical')
    ...
    for red in part_snapshots:
        ...
        r_z0 = part_z0['star'].prop('host2.distance.principal.cylindrical')
    ...
    ut_io.file_hdf5('/home/rlgraf/Final_Figures/surface_density_profile_star_raw_no_t0', surf_dens_ratio_gas_all_galaxies)
