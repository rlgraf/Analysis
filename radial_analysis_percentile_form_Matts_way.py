#!/usr/bin/env python3
#SBATCH --job-name=half_mass_radius_analysis_percentile_Matts_way
...
#SBATCH --output=half_mass_radius_analysis_percentile_form_Matts_way_%j.txt
...
def half_mass_radius_form(x1,x2,a1,a2,r_spherical,age,part):
    ...
    index = ut.array.get_indices(r_spherical[:,0], [x1,x2])
    index2 = ut.array.get_indices(age, [a1,a2], prior_indices = index)
    if len(r_spherical[:,0][index2]) == 0:
        return(np.nan)
    else:
        mass_cut = ut.math.percentile_weighted(r_spherical[:,0][index2], 90, weights = part['star']['mass'][index2])
        return(mass_cut)
...
def half_mass_radius_analysis_form():
    ...
    for q, s in enumerate(sim):
        ...
        for j, r_spherical in enumerate(r_array_spherical):
            half_mass_radius_at_age = []
            LG_counter += j
            for a in np.arange(0,14):
                half_mass_radius_at_age.append(half_mass_radius_form(0,30,a,14,r_spherical,age,part))
            half_mass_radius_galaxy.append(half_mass_radius_at_age)
    half_mass_radius_galaxy = np.array([half_mass_radius_galaxy])
    ut_io.file_hdf5('/home/rlgraf/Final_Figures/90_mass_radius_Matts_way', half_mass_radius_galaxy)
