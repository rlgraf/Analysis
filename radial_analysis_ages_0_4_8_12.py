#!/usr/bin/env python3
#SBATCH --job-name=radial_analysis_location_cut_ages_0_4_8_12
...
#SBATCH --output=radial_analysis_location_cut_ages_0_4_8_12%j.txt
...
def R90_func():
    ...
    R90_stack_0_4 = R90_stack[:,:4]
    R90_stack_4_8 = R90_stack[:,4:8]
    R90_stack_8_12 = R90_stack[:,8:12]

    R90_0_4_mean = np.nanmean(R90_stack_0_4,1)
    R90_4_8_mean = np.nanmean(R90_stack_4_8,1)
    R90_8_12_mean = np.nanmean(R90_stack_8_12,1)

    R90_0_4_8_12_array = np.array([np.nanmean(R90_0_4_mean), np.nanmean(R90_4_8_mean), np.nanmean(R90_8_12_mean)])
 
    return(R90_0_4_8_12_array)
...
def radial_analysis_z_0():
    ...
    for q, s in enumerate(sim):
        ...
        for j, (r, r_form) in enumerate(zip(r_array,r_form_array)):
            ...
            LG_counter += j
            r90 = R90
            for a, b in zip(np.arange(0,12,4), r90):
                x = []
    ...
    ut_io.file_hdf5('/home/rlgraf/Final_Figures/RAD_profile_z_0_ages_0_4_8_12', Fe_H_rad_total)
    ut_io.file_hdf5('/home/rlgraf/Final_Figures/RAD_slope_z_0_ages_0_4_8_12', slope_total)
...
def radial_analysis_form():
    ...
    for q, s in enumerate(sim):
        ...
        for j, (r, r_form) in enumerate(zip(r_array,r_form_array)):    
            ...
            LG_counter += j
            r90 = R90
            for a_f, b_f in zip(np.arange(0,12,4), r90):
                x_f = []
                for i_f in np.arange(0,b_f,b_f/10):
                    x_f.append(Fe_H_agedependent_form(i_f,i_f+b_f/10,-3,3,0,30,-3,3,a_f,a_f+1,r_form,r,age,part))
    ...
    ut_io.file_hdf5('/home/rlgraf/Final_Figures/RAD_profile_form_ages_0_4_8_12', Fe_H_rad_form_total)
    ut_io.file_hdf5('/home/rlgraf/Final_Figures/RAD_slope_form_ages_0_4_8_12', slope_form_total)
