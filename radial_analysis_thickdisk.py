...
#SBATCH --output=radial_analysis_thickdisk_%j.txt
    ...
    for q, s in enumerate(sim):
        ...
        for j, (r, r_form) in enumerate(zip(r_array,r_form_array)):    
            ...
            for a_f, b_f in zip(np.arange(0,14), r90):
                ...
                x_f = np.array(x_f)
                if len(x_f) == 0:
                    nan_array_f = np.array([np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan])
                    slope.append(nan_array_f)
                else:
