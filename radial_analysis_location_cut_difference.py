#!/usr/bin/env python3
#SBATCH --job-name=radial_analysis_location_cut_difference
...
#SBATCH --output=radial_analysis_location_cut_difference%j.txt
...
def radial_analysis_z_0():
    ...
    for q, s in enumerate(sim):
        ...
        for j, (r, r_form) in enumerate(zip(r_array,r_form_array)):
            ...
            for a, b in zip(np.arange(0,14), r90):
                x = []
                for i in np.arange(0,R90_z_0[q+LG_counter],R90_z_0[q+LG_counter]/50):
                    x.append(Fe_H_agedependent(i,i+R90_z_0[q+LG_counter]/50,-3,3,0,b,-3,3,a,a+1,r,r_form,age,part))
                Fe_H_rad.append(x)
                l = np.arange(0,R90_z_0[q+LG_counter],R90_z_0[q+LG_counter]/50)
                x = np.array(x)
                if np.isnan(x).all():
                    slope.append(np.nan)
                else:
                    j = (x[49] - x[0])/R90_z_0[q+LG_counter]
                    slope.append(j)
    ...
    ut_io.file_hdf5('/home/rlgraf/Final_Figures/RAD_profile_z_0_location_cut_difference', Fe_H_rad_total)
    ut_io.file_hdf5('/home/rlgraf/Final_Figures/RAD_slope_z_0_location_cut_difference', slope_total)
...
def radial_analysis_form():
        ...
        for j, (r, r_form) in enumerate(zip(r_array,r_form_array)):    
            ...
            for a_f, b_f in zip(np.arange(0,14), r90):
                ...
                for i_f in np.arange(0,b_f,b_f/50):
                    x_f.append(Fe_H_agedependent_form(i_f,i_f+b_f/50,-3,3,0,30,-3,3,a_f,a_f+1,r_form,r,age,part))
    ...
    ut_io.file_hdf5('/home/rlgraf/Final_Figures/RAD_profile_form_location_cut_difference', Fe_H_rad_form_total)
    ut_io.file_hdf5('/home/rlgraf/Final_Figures/RAD_slope_form_location_cut_difference', slope_form_total)
