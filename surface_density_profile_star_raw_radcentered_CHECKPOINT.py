#!/usr/bin/env python3
#SBATCH --job-name=surface_density_profile_star_raw_radcentered_CHECKPOINT
...
#SBATCH --output=surface_density_profile_star_raw_radcentered_CHECKPOINT_%j.txt
...
def surf_dens_log_frac(x1,x2,x3,x4,r,part,particle_thresh = 4):
    
    
    index = ut.array.get_indices(r[:,0], [x1,x2])
    index2 = ut.array.get_indices(abs(r[:,2]), [x3,x4], prior_indices = index)
    if len(part['star']['mass'][index2] < particle_thresh:
           return(np.nan)
           
    surf_dens_lookback = np.sum(part['star']['mass'][index2])/(np.pi*(x2**2 - x1**2))
...
### m12i
    ...
    for red in part_snapshots:
        ...
        for i in np.arange(0.5,15):
            x.append(surf_dens_log_frac(i,i+1,-3,3,r,part))
    ...
    ut_io.file_hdf5('/home/rlgraf/Final_Figures/surface_density_profile_star_raw_radcentered_CHECKPOINT', surf_dens_ratio_gas_all_galaxies)
