#!/usr/bin/env python3
#SBATCH --job-name=radial_analysis_0_12_FINAL
...
#SBATCH --output=radial_analysis_0_12_FINAL_%j.txt
...
def radial_analysis_z_0():
    ...
    for q, s in enumerate(sim):
        ...
        for j, (r, r_form) in enumerate(zip(r_array,r_form_array)):
            ...
            for a, b in zip(np.arange(0,14), r90):
                x = []
                for i in np.arange(0,12,12/50):
                    x.append(Fe_H_agedependent(i,i+12/50,-3,3,0,30,a,a+1,r,r_form,age,part))
                Fe_H_rad.append(x)
                l = np.arange(0,12,12/50)
                x = np.array(x)
                if np.isnan(x).all():
                    slope.append(np.nan)
                else:
    ...
    ut_io.file_hdf5('/home/rlgraf/Final_Figures/RAD_profile_z_0_location_cut_median_weighted_0_12_FINAL', Fe_H_rad_total)
    ut_io.file_hdf5('/home/rlgraf/Final_Figures/RAD_slope_z_0_location_cut_median_weighted_0_12_FINAL', slope_total)


# formation

def Fe_H_agedependent_form(x1,x2,x3,x4,x5,x6,a1,a2,r_form,r,age,part, particle_thresh = 16):
...
def radial_analysis_form():
    ...
    for q, s in enumerate(sim):
        ...
        for j, (r, r_form) in enumerate(zip(r_array_spherical,r_form_array)):    
            ...
            for a_f, b_f in zip(np.arange(0,14), r90):
                x_f = []
                for i_f in np.arange(0,12,12/50):
                    x_f.append(Fe_H_agedependent_form(i_f,i_f+12/50,-3,3,0,30,a_f,a_f+1,r_form,r,age,part))
                Fe_H_rad_form.append(x_f)
                l_f = np.arange(0,12,12/50)
                x_f = np.array(x_f)
                if np.isnan(x_f).all():
    ...
    ut_io.file_hdf5('/home/rlgraf/Final_Figures/RAD_profile_form_location_cut_median_weighted_0_12_FINAL', Fe_H_rad_form_total)
    ut_io.file_hdf5('/home/rlgraf/Final_Figures/RAD_slope_form_location_cut_median_weighted_0_12_FINAL', slope_form_total)
