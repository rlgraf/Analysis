#!/usr/bin/env python3
#SBATCH --job-name=radial_analysis_surface_density_ratio_and_Fe_H_100Myr_thirdR98_R98_young
...
#SBATCH --output=radial_analysis_surface_density_ratio_and_Fe_H_100Myr_thirdR98_R98_young_%j.txt
...
def radial_analysis_form():
    ...
    for (red,r98) in zip(part_snapshots,R98_young_m12i):
        ...
        x_f = []
        surf_dens_ratio = []
        for i_f in np.arange(r98,r98,(r98 - r98/3)/20):
            x_f.append(Fe_H_agedependent_form(i_f,i_f+(r98 - r98/3)/20,-3,3,0,0.1,r_star,age,part))
            surf_dens_ratio.append(log_surf_dens_ratio(i_f,i_f+(r98 - r98/3)/20,-3,3,0,0.1,r_star,r_gas,age,part))
            
        Fe_H_rad_form_at_snapshot.append(x_f)
        l_f = np.arange(r98/3,r98,(r98 - r98/3)/20)
        x_f = np.array(x_f)
        if np.isnan(x_f).all():
        ...
        l_f_s = np.arange(r98/3,r98,(r98 - r98/3)/20)
    ...
    for (red,r98) in zip(part_snapshots,R98_young_m12c):
        ...
        x_f = []
        surf_dens_ratio = []
        for i_f in np.arange(r98/3,r98,(r98 - r98/3)/20):
            x_f.append(Fe_H_agedependent_form(i_f,i_f+(r98 - r98/3)/20,-3,3,0,0.1,r_star,age,part))
            surf_dens_ratio.append(log_surf_dens_ratio(i_f,i_f+(r98 - r98/3)/20,-3,3,0,0.1,r_star,r_gas,age,part))
    ...
    ut_io.file_hdf5('/home/rlgraf/Final_Figures/Fe_H_rad_form_all_galaxies_surfdensratiotrack_100Myr_thirdR98_R98_young', Fe_H_rad_form_all_galaxies)
    ut_io.file_hdf5('/home/rlgraf/Final_Figures/surf_dens_ratio_all_galaxies_surfdensratiotrack_100Myr_thirdR98_R98_young', surf_dens_ratio_all_galaxies)
    
    ut_io.file_hdf5('/home/rlgraf/Final_Figures/Fe_H_rad_form_all_galaxies_surfdensratiotrack_100Myr_thirdR98_R98_young_slope', Fe_H_rad_form_all_galaxies_slope)
    ut_io.file_hdf5('/home/rlgraf/Final_Figures/surf_dens_ratio_all_galaxies_surfdensratiotrack_100Myr_thirdR98_R98_young_slope', surf_dens_ratio_all_galaxies_slope)
