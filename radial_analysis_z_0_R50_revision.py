#!/usr/bin/env python3
#SBATCH --job-name=radial_analysis_z_0_R50_revision
...
#SBATCH --output=radial_analysis_z_0_R50_revision_%j.txt
...
R50 = np.array([2.6, 2.9, 2.9, 4.3, 2.2, 3.6, 1.8, 3.2, 2.9, 3.4, 2.8])
...
def radial_analysis_z_0():
    ...
    sim = sim_func()
    LG_counter = 0
    for q, (s,r50) in enumerate(zip(sim, R50)):
        ...
        for j, (r, r_form) in enumerate(zip(r_array,r_form_array)):
            ...
            for a in np.arange(0,14):
                x = []
                for i in np.arange(0,r50,r50/50):
                    x.append(Fe_H_agedependent(i,i+r50/50,-3,3,0,30,a,a+1,r,r_form,age,part))
                Fe_H_rad.append(x)
                l = np.arange(0,r50,r50/50)
                x = np.array(x)
                if np.isnan(x).all():
                    slope.append(np.nan)
                else:
